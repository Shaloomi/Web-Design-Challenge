...
3. Inside your local Git repository, add your HTML files along with three folders; `assets`, `Resources` and `visualisations`. 
  ...
  * The `Resources` folder should contain the CSV file with the data you are using for this website. The `visualisations` folder will contain the HTML pages that display your visualisations.
...
For this homework assignment, you'll create a website by using visualisations that were created in your Python-APIs homework, or you can use the [weather data](Resources/cities.csv) provided.

As you build this dashboard, you'll create individual pages for each plot and a way to navigate between them. These pages will contain the visualisations and descriptions. You will also build a landing page to provide a comparison of all the plots, along with another page to present the data used to build them.
...
* A [landing page](#landing-page) containing the following elements:
  ...
  * Links to each visualisation page. There should be a sidebar containing preview images of each plot. Clicking an image should take the user to that visualisation.

* Four [visualisation pages](#visualisation-pages), stored in the `visualisations` folder, each with the following elements:
...
* A ["Comparisons" page](#comparisons-page) that does the following:
  ...
  * Uses a Bootstrap grid for the visualisations.
    ...
    * The grid must be two visualisations across medium and large screens, and it must be one visualisation across on extra-small or small screens.

* A ["Data" page](#data-page) that displays a responsive table containing the data used in the visualisations.
...
* It should be responsive (using media queries). The navigation bar must be similar to the screenshots in the ["Navigation Menu" section](#navigation-menu) (notice the background colour change).
...
* Use a Bootstrap theme to customise your website. You can use a tool like [Bootswatch](https://bootswatch.com/), but make sure you also meet all of the requirements listed earlier.

* Add extra visualisations. 
* Use meaningful glyphicons next to the links in the header.

* Have visualisation navigation on every visualisation page with an active state, as in the following screenshots.
...
You'll build four of these, one for each visualisation. Here's an example of one visualisation page on two different screen sizes:

Large screen:

![visualise page large screen](Images/visualise-lg.png)

Small screen:

![visualise page small screen](Images/visualise-sm.png)
...
>>>>>>> 0f9b0cef431c53657c7d00b58ae6daf269930870
